...
6. **Hash credentials (hash field present, no plaintext password):**
   - Check hash type: MD5/bcrypt = web app hash, NTLM = domain hash
   - Web hashes (MD5/bcrypt): spawn `kerberoast` agent with crack_hash focus BEFORE lateral routing
   - NTLM hashes: route directly to `lateral` with pass-the-hash
   - Do NOT route uncracked web hashes to lateral movement (they can't authenticate to SMB/WinRM)
...
| Reporting | report | DO NOT SPAWN -- auto-generated by pipeline |
